...
LLM client opens a review session; `ReviewerChannelBackend` handles QUERY messages via `DocumentReviewer @AiService` and dispatches RESPONSE/DECLINE to the Qhorus channel.
...
**What this delivers**
🔲 at #27 close — Qhorus `DebateChannel` type; AGREE/QUALIFY sub-classification; `SummaryRenderer` implements `ProjectionRenderer` SPI (qhorus#232 ✅ shipped; migration tracked in #31).
